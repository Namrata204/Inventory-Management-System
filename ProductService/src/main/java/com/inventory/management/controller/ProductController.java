...
public class ProductController {
     ...
     * @param quantity    The updated stock quantity as a request parameter.
     * @return HTTP status 200 (OK) if successful,
     *         or HTTP status 404 (Not Found) if the product does not exist.
     */
    @PutMapping("/{productName}/update-stock")
    public ResponseEntity<Void> updateStockQuantity(
            @PathVariable String productName,
            @RequestParam("quantity") int quantity
    ) {
        productService.updateStockQuantity(productName, quantity);
